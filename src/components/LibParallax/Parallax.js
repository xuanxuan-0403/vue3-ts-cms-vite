export default class {
    ...
    moveEvent() {
        ...
        this.parent.addEventListener('mousemove', (e) => {
            window.requestAnimationFrame(() => {
                ...
                if (this.follow) {
                    this.x = Number(
                        ((flag * (e.clientX - this.startX)) / this.parent.clientWidth).toFixed(2),
                    );
                    this.y = Number(
                        ((flag * (e.clientY - this.startY)) / this.parent.clientHeight).toFixed(2),
                    );
                } else {
    ...
    enable() {
        for (const { el, options } of this.options) {
            ...
            if (this.follow) {
                if (blur !== false) {
                    el.style.filter = `blur(${Math.abs(
                        -initblur + Math.abs(this.x) * initblur + blur * Math.abs(this.x),
                    )}px)`;
                } else {
                    el.style.filter = `blur(${initblur}px)`;
                }
